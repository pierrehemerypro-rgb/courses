...
export default function App() {
  ...
  const [plan, setPlanState]      = useState(genererPlan);
  ...
  useState(() => {
    (async () => {
      try {
        ...
        if (savedPlan) setPlanState(JSON.parse(savedPlan.value));
      } catch {}
      try {
  ...
  const genererCourses = async () => {
    setGenerating(true);
    setCourses(null);
    // On ne compte les restes qu'une fois (pas d'ingrédients supplémentaires)
    ...
    // Pour les wraps on compte 3 repas → paquet de 6 wraps
    const wrapCount = plan.flatMap(j => j.repas).filter(r => r.nom === WRAP).length;
    const ingredients = nomsUniques.map(n => pool.find(r => r.nom === n)).filter(Boolean).flatMap(r => r.ingredients);
    const prompt = `Tu es un assistant cuisine. Liste brute d'ingrédients pour une semaine de repas maison (1 personne). 
Règles importantes pour le packaging :
- Les wraps s'achètent par paquet de 6 → indiquer "1 paquet (6 wraps)" même si on en utilise ${wrapCount * 2} (2 par repas)
- Les falafels Herta s'achètent en sachet de 8-10 → indiquer "1 sachet"
- Les escalopes végé s'achètent par 2-3 → indiquer le bon nombre de paquets
- Les œufs s'achètent par 6 ou 12 → arrondir au paquet supérieur
- Pour les légumes frais, indiquer le poids ou le nombre réaliste à acheter
- Consolide les doublons (ex: plusieurs recettes avec carottes → 1 ligne "carottes Xkg")
- Pour les recettes qui font 2 portions (marquées 🔁 Restes), les ingrédients ne sont comptés qu'une seule fois

Réponds UNIQUEMENT en JSON valide sans texte ni balises markdown :
{"rayons":[{"nom":"🥦 Fruits & Légumes","items":[{"nom":"...","quantite":"..."}]},{"nom":"🧀 Frais / Réfrigéré","items":[]},{"nom":"🥫 Épicerie sèche","items":[]},{"nom":"❄️ Surgelés / Végé","items":[]},{"nom":"🌿 Épices & Condiments","items":[]},{"nom":"🍞 Boulangerie & Autres","items":[]}]}
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        headers: {
          ...
          ...(import.meta.env?.VITE_ANTHROPIC_API_KEY ? { "x-api-key": import.meta.env.VITE_ANTHROPIC_API_KEY } : {})
        },
